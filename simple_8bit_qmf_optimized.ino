...
void loop() {
  ...
  // Scale down the outputs by (2^15 / 4) and clamp them to -63 to +63 range
  y1 = constrain(y1 >> 17, -63, +63); 
  y2 = constrain(y2 >> 17, -63, +63);

  // Write the output buffers to the PWM pins (scaled to 0-255)
  analogWrite(LP_PIN, map(y1, -63, +63, 0, +255)); // Lowpass band
  analogWrite(HP_PIN, map(y2, -63, +63, 0, +255)); // Highpass band

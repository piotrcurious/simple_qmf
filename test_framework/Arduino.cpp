...
void advance_time(uint32_t c) { total_cycles += c; }
...
int analogRead(uint8_t pin) {
    add_cycles(128); // Realistic AVR analogRead (~8us)
    if (pin == A1) {
        int val;
...
void analogWrite(uint8_t pin, int val) {
    add_cycles(8); // Realistic
    printf("%d %d\n", (int)pin, val);
}

void delay(unsigned long ms) { advance_time((uint64_t)ms * 16000); }
void delayMicroseconds(unsigned int us) { advance_time((uint64_t)us * 16); }
